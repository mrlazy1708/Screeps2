...

{
  ...
  const size = 0.8*Math.max(document.body.offsetWidth, document.body.offsetHeight);
  canvas.style.width = `${size}px`;
  canvas.style.height = `${size}px`;
  canvas.style.backgroundColor = "#2b2b2b";
  const left = 0.5 * (document.body.offsetWidth - size);
  canvas.style.left = `${left}px`;
  const top = 0.5 * (document.body.offsetHeight - size);
